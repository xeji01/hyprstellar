...
check_battery_status() {
    ...
    if [ "$capacity" -le 20 ] && [ "$previous_status" != "low" ]; then
         notify-send -u critical -i "$ICON_PATH/battery-low-48.png" "Battery at 20%" "Please plug in the charger."
        previous_status="low"
    elif [ "$capacity" -eq 100 ] && [ "$previous_status" != "full" ]; then
         notify-send -u normal -i "$ICON_PATH/battery-full-48.png" "Battery at 100%" "Your battery is fully charged. You can unplug your device."
        previous_status="full"
    fi

    # Check for plugging/unplugging events
    if [ "$status" == "Charging" ] && [ "$previous_power_status" != "plugged" ]; then
        notify-send -u normal -i "$ICON_PATH/battery-charging-48.png" "Charging" "Charger is plugged in."
        previous_power_status="plugged"
    elif [ "$status" == "Discharging" ] && [ "$previous_power_status" != "unplugged" ]; then
        notify-send -u normal -i "$ICON_PATH/battery-unplugged-48.png" "Discharging" "Charger is unplugged."
